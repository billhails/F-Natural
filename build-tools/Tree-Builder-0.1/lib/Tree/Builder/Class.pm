package Tree::Builder::Class;

use strict;
use warnings;
use Tree::Builder::Scalar;
...
Tree::Builder::Data - Tree Data

=head1 SYNOPSIS

  use Tree::Builder::Parser;
  my $parser = new Tree::Builder::Parser();
...
=head2 new
  ...
  my $class = new Tree::Builder::Class(%params);
...
will be passed to L<Tree::Builder::Args::List()>
...

{
    ...
    sub new {
        ...
        unless (exists $symbols{$name}) {
            if ($name eq 'scalar') {
                return new Tree::Builder::Scalar();
            } else {
                $symbols{$name} = bless {
                    _name => $name,
                    _substantial => 0,
        ...
        if ($params{args}) {
            ...
            if ($self->{_args}) {
                ...
                $self->{_args} = Tree::Builder::Args->List($params{args});
            }
        }
        if ($params{abstract}) {
            $self->{_abstract} = 1;
...
Each element is a C<Tree::Builder::Args>.
