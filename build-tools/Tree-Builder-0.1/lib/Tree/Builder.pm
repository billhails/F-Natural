...
Tree::Builder - Build Classes from a Tree Definition File

=head1 SYNOPSIS

  use Tree::Builder;
  my $builder = new Tree::Builder(%params);
  $builder->build($config_file);

=head1 DESCRIPTION

C<Tree::Builder> takes a set of parameters and a tree definition file
...
sub getData {
    ...
    my $parser = Tree::Builder::Parser->new(prefix => $self->{prefix});
...
sub makeSpecialClass {
    my ($self, $class) = @_;
    new Tree::Builder::Class(class => ($self->{prefix} . $class));
}
...
# returns the output filename for an argument Tree::Builder::Class
